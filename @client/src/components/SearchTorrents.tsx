...
export const SearchTorrents = () => {
  ...
  if (frm.getValues().query.length === 0 && q.query) setQuery({ ...q, query: '' })
      ...
      {torrents.data?.map((x) => (
        <article className="rounded-lg border border-gray-100" key={x.magnet}>
          <div className="flex flex-wrap pt-5 pr-5">
            <h3 className="text-lg font-medium w-full pt-5 pl-5 min-w-24 flex-[1_1_16rem] break-words">{x.title}</h3>
