...
const Torrent = ({ id }: { id: string }) => {
  ...
  const title = (() => {
    ...
    const p = ptn(data.data.name)
    let title = [p.title]
    if (typeof p.season !== 'undefined') title.push(`S${p.season}`)
    if (typeof p.episode !== 'undefined') title.push(`E${p.episode}`)
    if (typeof p.resolution) title.push(`(${p.resolution})`)
    return title.join(' ')
  })()

  return (
    <article className="rounded-lg border border-gray-100 bg-white p-4 relative">
      <p className="text-xs absolute -top-2 left-0 px-5 opacity-20 truncate w-full">{data.data.id}</p>
      <div className="flex justify-between">
        <p className="pb-2 break-words w-full">{title}</p>
        <div
