...
export const Ec2 = (stack: AppStack, props: { bucket: cdk.aws_s3.IBucket }) => {
  const vpc = new cdk.aws_ec2.Vpc(stack, stack.id('Vpc'), {
    // cidr: '10.0.0.0/16',
    natGateways: 0,
    subnetConfiguration: [{ name: 'public', cidrMask: 24, subnetType: cdk.aws_ec2.SubnetType.PUBLIC }],
  })
  ...
  const role = new cdk.aws_iam.Role(stack, 'Role', { assumedBy: new cdk.aws_iam.ServicePrincipal('ec2.amazonaws.com') })
  ...
  const securityGroup = new cdk.aws_ec2.SecurityGroup(stack, stack.id('SimpleInstanceSG'), {
    vpc: vpc,
